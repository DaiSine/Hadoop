		...
		try {
			...
			if ( !HDFSApi8.test(conf, remoteFilePath) ) {
				...
				if ( choice.equals("after") ) { // 追加在文件末尾
					...
					HDFSApi8.moveToLocalFile(conf, remoteFilePath, localTmpPath);  // 移动到本地
					HDFSApi8.touchz(conf, remoteFilePath);    // 创建一个新文件
					HDFSApi8.appendContentToFile(conf, content, remoteFilePath);   // 先写入新内容
					HDFSApi8.appendToFile(conf, localTmpPath, remoteFilePath);   // 再写入原来内容
